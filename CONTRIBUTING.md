# Contributing to ep_public_view

Thanks for helping improve `ep_public_view` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_public_view` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_public_view from your local clone
pnpm run plugins i --path ../ep_public_view

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_public_view/` are picked up after a server restart.
...
* Etherpad version, ep_public_view version, browser + OS
* Steps to reproduce
